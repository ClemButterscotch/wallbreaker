...
const slides=[
  ...
  },
  {
    ...
    facts:['The host waits until every player is locked, then adds all legal effects together before revealing the result.','On one dial, +2 and +1 produce a public change of +3. A +1 and a −1 produce 0, even though both players had to act.','The public reveal shows only each dial’s net change, not the individual actions that produced it.','Passing and zero-effect moves are not legal. When the host chooses observer mode, messaging is private between each player and the observer.']
  },
  {
...
export function briefingHtml({state,step,isHost,isPlayer,myPlayerId}){
  ...
  const readyAction=`<button id="tutorial-ready" class="tutorial-primary" ${amReady?'disabled':''}>${amReady?'Ready':'I understand — ready'}</button>`;
  const finalAction=isHost
    ? `${isPlayer?readyAction:''}<button id="begin-tutorial-practice" class="tutorial-primary">Begin guided practice</button><span>${ready.size}/${state.players.length} players ready</span>`
    : readyAction;
  return `<div class="tutorial-shell"><div class="tutorial-aurora" aria-hidden="true"></div><header class="tutorial-header"><div><div class="product-brand"><img class="brand-mark" src="/logo.svg" alt="" width="28" height="28"><div class="brand">WALLBREAKER</div></div><div class="meta">Guided rules · Room ${esc(state.code)}</div></div><button class="tutorial-exit" id="leave">${isHost?'End game':'Leave game'}</button></header><main class="tutorial-stage" aria-live="polite"><section class="tutorial-copy"><div class="tutorial-eyebrow">${slide.eyebrow}</div><h1>${slide.title}</h1><p class="tutorial-lede">${slide.copy}</p><ul>${slide.facts.map(fact=>`<li>${fact}</li>`).join('')}</ul></section><section class="tutorial-art" aria-label="Illustration for ${esc(slide.title)}">${slide.art}</section></main><footer class="tutorial-controls"><button class="tutorial-back" id="tutorial-prev" ${index===0?'disabled':''}>← Back</button><div class="tutorial-dots">${dots}</div>${index<slides.length-1?'<button class="tutorial-primary" id="tutorial-next">Continue →</button>':`<div class="tutorial-final-action">${finalAction}</div>`}</footer>${index===slides.length-1?`<aside class="tutorial-ready-roster"><strong>${isHost?'Briefing status':'Your group'}</strong><div>${roster}</div></aside>`:''}</div>`;
}
